	...
	if (aoeCnt > 0 && !fE && !fB && ndmg > 0) {
		...
		while ((p = v_each(pers->fight->persVec, &vi))) {
			...
			if (!pers_is_alive(p) && p->status != FS_PS_DEAD) {
				fs_persDie(p, pers);	// kill immediately so pairing loop skips them
			}
...
double fs_persDamageEx(fs_pers_t *pers, double dmg, int dmgType, bool crit, fs_pers_t *activator, bool noEnhance) {
	...
	if (PERS_HP(pers) <= 0) {
		...
		// fs_persDie вызовется из fs_persRecalcEffects — это безопасно
	}
	...
	while ((p = v_each(&v1,0))) {
		...
		if (((i == 0) || (i == 20) || (i == 30)) && (eff->dmg > 0)) {	// applying unconditional effect damage
			if ((i == 0) || (i == 30)) {
				...
				fs_persRecalcEffects(p);   // уже есть
				...
				if (pers_is_alive(pers)) fs_persRecalcEffects(pers);
		...
		switch (eff->code) {
				...
				for (skill = 0; skill<=FS_SK_MAXCODE; skill++) {
					...
					if ((!fTime && fStart) || (fPeriod && fTrigger)) {
						if (skill == FS_SK_HP) {
							...
							fs_persDamage(pers, (eff->dmgRecalc ? eff->dmgRecalc : -val), eff->dmgType, false, healer);
			...
			case FS_PEC_ADDHP:
				if ((!fTime && fStart) || (fPeriod && fTrigger)) {
					...
					if ((eff->dmg <= 0) && (eff->dmgType & ~OLD_DMGTYPE) && fStart && fPeriod

					&& (val < 0) && eff->activator && (ticks > 0)) {
						eff->dmgRecalc = fs_persRecalcDmg(eff->activator, pers, eff->dmgType,
															(-val * ticks), 1, eff->aoeCnt) / ticks;
						eff->dmgRecalc = MAX(eff->dmgRecalc, 0.1);
	 }

					// DOT_DMG_MULT: плоский % к периодическому урону (все типы)
					// Вычисляется один раз при fStart, записывается в dmgRecalc
					if (fStart && fPeriod && (val < 0) && eff->activator && (ticks > 0) && !eff->f3)  {
						int dotMult = fs_clamp_skill(FS_SK_DOT_DMG_MULT,
													  PERS_SKILL(eff->activator, FS_SK_DOT_DMG_MULT));
						if (dotMult > 0) {
							// Если dmgRecalc ещё не задан (физический DoT) — берём из val
							if (!eff->dmgRecalc) eff->dmgRecalc = -val;
							eff->dmgRecalc *= (1.0 + dotMult / 100.0);
							eff->dmgRecalc = MAX(eff->dmgRecalc, 0.1);
						}
					}
